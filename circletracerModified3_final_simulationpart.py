...
def init_obstacles():  # Later run in for loop
    for q in obst:
        cylinder(color=green, pos=vector(q[0], q[1], 0), axis=vector(
            0, 0, 200), radius=q[2]+gr, opacity=0.8)
        cylinder(color=maroon, pos=vector(
            q[0], q[1], 0), axis=vector(0, 0, 200), radius=q[2])
        q[2] = q[2] + 10
...
def optimalheight(path):
    ...
    for j in range(len(path)-2):
        temp = path[0][-1] + (path[-1][-1] - path[0][-1]) * \
            abs_dis[j]/abs_dis[-1]
        h.append(temp)
...
def reset():
    ...
    screen.camera.pos = vector(500, 1000, 450)
    # screen.center = vector(450, 300, 0)
    screen.camera.axis = vector(0, -600, -400)
    screen.up = vector(1, -400, 600)

    box(pos=vector(350, 450, 0), size=vector(900, 700, 0.1), color=color.white)
    init_obstacles()
...
while True:
    ...
    goalset = 0
    while goalset < n:
        v = vector(destiny[goalset][0], destiny[goalset]
                   [1], destiny[goalset][2])
        sphere(color=red, pos=v, radius=6)
        goalset += 1
    ...
    while True:
        ...
        m = [NextPoint[0]-FirstPoint[0], NextPoint[1] -
             FirstPoint[1], NextPoint[2]-FirstPoint[2]]
        ...
        destiny[0] = [FirstPoint[0] + dList[0],
                      FirstPoint[1] + dList[1], FirstPoint[2] + dList[2]]
        print(destiny[0])
